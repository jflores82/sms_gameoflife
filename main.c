...
int cellCheckNeighboor(int x, int y, int cellStatus) { 
    ...
    if(cellStatus == 1) {
        ...
        if(neighboors >= 3 && neighboors <= 5) { alive = 1; } // cell is alive //
        if(neighboors > 5) { alive = 0; } // overpopulated //
        return alive;
    }
...
// Stats Screens //
void statsRender(int alive) {
    SMS_setNextTileatXY(4,4);
    printf("Generations: %u", gen);
    SMS_setNextTileatXY(4,6);
    printf("Alive: %u", alive);
}
